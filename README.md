...
3. 執行 `./scripts/setup-obsidian-mcp.sh`，把 Obsidian MCP server 複製到 `~/.codex/mcp/` 並重新簽章。
...
7. `install-mac.sh` 只安裝可攜工具與共用模板，不直接改 live 排程，並會印出這次的本機 baseline 備份路徑；由 Codex automation tool 補齊或更新排程後，必須用該路徑跑 strict baseline audit。
8. 兩台 Mac 應有同一組排程，但各自保存 `ACTIVE`／`PAUSED`、target 與 cwd；任何一台的開關都不得覆寫另一台。
...
git status --short -- AGENTS.md rules skills agents-skills automations-templates automation-tools scripts tests README.md MANIFEST.md OTHER-MAC-CODEX-HANDOFF.md
git diff -- AGENTS.md rules skills agents-skills automations-templates automation-tools scripts tests README.md MANIFEST.md OTHER-MAC-CODEX-HANDOFF.md
```
...
這會把另一台 Mac 的 `AGENTS.md`、repo 已追蹤名稱的 command rules、`~/.codex/skills` 與 `~/.agents/skills` 收集到 `incoming/`，方便之後比較。完整 automation host-state 會另存到該台 Mac 的 `~/.codex-env-backups/merge-review/`，不會進 Git，也不得安裝到另一台。確認要保留哪些規則或 skill 差異後，再把需要的檔案合併回 `rules/`、`skills/`、`agents-skills/` 或 `AGENTS.md`；共用排程定義必須另外明確編輯並驗證。
