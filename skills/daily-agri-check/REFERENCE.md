...
- 固定 PDF：`https://www.cpc.ncep.noaa.gov/products/analysis_monitoring/lanina/enso_evolution-status-fcsts-web.pdf`。目錄名稱固定為 `lanina`，不隨 ENSO 相位更名。
- 固定檔名每週覆寫，以 HTTP `Last-Modified`、PDF 首頁日期或頁內 `Update prepared` 判斷新舊，不用檔名判斷。
- PDF 暫時不可讀時，改抓官方 relative 純文字：`https://www.cpc.ncep.noaa.gov/data/indices/rel_wksst9120.txt`，取最後一列。不得混用傳統 anomaly 的 `wksst9120.for`；日報與 tracker 統一使用 NOAA 現行 relative／RONI 口徑。

### UNICA Center-South 雙週報

- 固定入口：`https://unicadata.com.br/listagem.php?idMn=63`。從頁面抽取第一個唯一的 `https://unicadata.com.br/arquivos/pdfs/YYYY/MM/<32位雜湊>.pdf`；雜湊檔名不可猜。
- 第二官方路徑：先讀 `https://unica.com.br/wp-sitemap.xml`，再找 `lastmod` 最新的 `noticias-sitemapN.xml` 與當期 crop news；不得永久硬編 sitemap 編號。
- 若固定入口仍指向上一期，將預期新一期標為 `scheduled but not yet published`，並把入口所示上一期保留為「最新正式報告」。只有入口與 PDF 都無法存取，才標 `unavailable`。

### CEPEA 水合乙醇

- 日值主頁：`https://cepea.org.br/en/indicator/ethanol.aspx`，讀 Paulínia daily table 的 `US$/m3`；這是 `CEPEA_USD_m3 * 0.030 + 2.5` 公式的直接輸入。
- 週值／序列備援：`https://cepea.org.br/br/indicador/etanol.aspx` 與 `https://cepea.org.br/br/indicador/series/etanol.aspx?id=103`。XLS 端點免登入，但命令列抓取需使用一般瀏覽器 User-Agent。
- 不再使用會逾時或 403 的舊 `cepea.esalq.usp.br` 主機。每日任務可把最新 CEPEA 平價寫入 sugar tracker 與晨報，但 `ethanol-parity-tracker` 仍由獨立任務維護，不得更新。
