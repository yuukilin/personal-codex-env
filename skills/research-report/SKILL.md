...
財報狗採「已登入 Chrome 優先」：必須優先用可互動的 Codex Browser/Chrome 工具連到使用者既有已登入 Chrome 狀態，開受控分頁讀取財報狗，讀完關閉分頁。不要把未登入公開頁或一般 web fetch 當主流程，因為公開頁連續看多頁可能觸發限制。不要索取或讀取使用者密碼、不要翻 Chrome cookie；若未登入或權限不足，停止並請使用者在 Chrome 登入，不要用公開頁硬刷，也不要用其他來源假裝補完。

財報狗動態頁標準讀取流程：用已登入 Chrome 開頁，等資料載入約 5 秒，看畫面是否有圖表或「詳細數據」，再讀文字層。若文字層抽不到但畫面有表格，改用表格區截圖/視覺讀表；DOM row/table count 只能輔助，不能當作資料不存在的判斷。若一種方法失敗，不要反覆重試同一招，直接換下一種方法。只有畫面與替代讀法都看不到核心資料，才停止並告知使用者；不要用其他來源假裝補完。
...
注意事項：
- 一般對話只要使用者問個股營收、月營收、EPS、毛利率、營益率、自由現金流、PE/PB、估值或法說會逐字稿，也套用同一條財報狗已登入 Chrome 優先規則。
...
美股法說會逐字稿：
- 用已登入 Chrome 開 `https://statementdog.com/analysis/{TICKER}/e-report`。
