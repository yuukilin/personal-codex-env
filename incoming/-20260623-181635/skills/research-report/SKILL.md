...
description: >
  針對特定公司或投資主題，結合使用者 Obsidian vault 歷史資料與網路最新資訊，撰寫完整深度研究報告（3000-6000字）並存入 vault。
  支援兩種模式：個股研究報告（如家登、台積電）和產業主題研究報告（如先進封裝、AI 電力需求）。
  Skill 自動判斷標的屬於哪種模式，依對應模板產出報告。
  此 Skill 僅在使用者訊息中出現「研究報告」三個字時觸發。
  常見觸發語：「研究報告 家登」「研究報告 先進封裝」「研究報告 2330」「研究報告 CoWoS」
  「幫我寫研究報告 NVDA」「研究報告 AI 電力」「研究報告 核能」。
  嚴格不觸發的情況：使用者沒有提到「研究報告」的任何訊息。
...
# Research Report — 深度研究報告產生器（v3 優化版）

## Codex 相容性

- 本 skill 中的 `Claude` 指目前的 Codex 助手。
- 本 skill 中的 Chrome MCP / `mcp__Claude_in_Chrome__*` 指可用的 Codex Chrome、Browser 或 Computer Use 工具；若登入狀態或動態表格需要真實瀏覽器，優先用 Chrome。
- 本 skill 中的 `web search` 指 Codex 可用的網路搜尋/瀏覽工具；涉及最新新聞、財報、股價、政策、產品規格時必須重新查證並附來源。
- 寫入 Obsidian 時優先使用 Obsidian MCP；若工具不可用，明確告知限制。

## 核心任務

使用者給出一個標的（公司名/代號/產業主題），Claude 需要一口氣完成以下流程，中間不停下來問使用者：

1. 窮盡搜尋使用者 Obsidian vault 中的歷史研究資料（兩層搜尋全做完）
2. 用財報狗 + web search 補充 vault 中不足或過時的資訊
3. 根據實際掌握的財務數據和股價狀態，精準套用投資智慧卡片
4. 整合所有資料，依對應模板撰寫 3000-6000 字的深度研究報告
5. 按標準格式直接存入 Obsidian vault（不問使用者）

重要流程規則：
- 使用者說「研究報告 XXX」就是要一份完整報告，從搜尋到動筆到產出，一路做到底
...
報告的核心價值：大量引用使用者自己的資料庫，串聯跨報告、跨時間的資訊，產出使用者獨有的研究視角。這不是一般的網路研究報告，而是從使用者累積的 500+ 份研究報告中提煉出的獨家整合分析。
...
個股研究報告 — 標的是一家特定公司（股票代號或公司名）
- 範例：家登、2330、NVDA、台積電、朋程(8255)

產業主題研究報告 — 標的是一個產業趨勢或技術主題
...
判斷完後告知使用者：「這份是個股/產業主題研究報告，讓我開始搜集資料。」然後立刻進入 Step 1，不要等使用者回覆。
...
5. 關鍵字補漏：用 search_vault_simple 搜尋公司中文名或產品名，但要優先限制資料夾
...
1. Theme 卡片：用 search_vault_simple 搜尋 Theme- 開頭的檔案，找到對應的卡片名，用 get_vault_file 讀取完整內容（特別注意「趨勢演變紀錄」區塊）
...
6. 關鍵字擴大搜尋：用 search_vault_simple 搜尋相關同義詞和子概念，但限制資料夾與結果數
...
台股個股報告必須使用財報狗（statementdog.com）抓取結構化財務數據。這是免費、即時更新的台股財務資料庫，數據品質高、格式統一。

使用可用的 Codex Chrome / Browser 工具開啟財報狗頁面。URL 格式：
https://statementdog.com/analysis/{股票代號}/{頁面路徑}

必訪頁面（依優先順序，至少訪問前 5 個）：
...
5. /product-revenue — 產品組合：各業務線的季度營收拆解。用來填寫第二節營收結構

視需要補充的頁面：

6. /eps — 每股盈餘：季度 EPS 歷史，搭配定錨財測做交叉驗證
...
10. /broker-trading — 董監籌碼：主力買賣超和籌碼集中度（波段交易參考）

抓取方式：用 navigate 進入頁面後，優先用 get_page_text 抓取表格數據（比截圖更精確、更省 token）。screenshot 用來看圖表趨勢（河流圖、走勢圖等需要視覺判斷的內容）。每個頁面的數據表格在頁面中段（chart 下方），預設顯示季報，可切換為年報。表格下方有產業排名比較，也值得記錄。

注意事項：
- 財報狗首頁（/analysis/{代號}）即可看到最新股價、漲跌幅、主要指標概覽
...
- 台股公司：先用可互動瀏覽器工具開財報狗抓取結構化財務數據（Step 2a 必做），再用 web search 補充新聞和法人觀點，優先找 MoneyDJ、CMoney、證交所等來源
- 美股公司：搜尋英文關鍵字，優先找 Seeking Alpha、Yahoo Finance、公司 IR 頁面
- 產業主題：同時搜中英文，找產業研究報告、新聞、分析

---

## Step 2.5：投資智慧卡片精準套用（Step 2 完成後才做）

此步驟必須在 Step 1（vault 搜尋）和 Step 2（財務數據抓取）都完成後才執行。因為只有掌握了完整的財務數據和股價表現，才能準確判斷哪些投資智慧卡片適用。

根據 Step 1 和 Step 2 取得的實際數據，判斷當前股票/主題的狀態，然後精準搜尋對應的卡片：
...
用 search_vault_simple 搜尋關鍵字找到相關卡片，每張只需讀取核心觀點（通常很短），不用讀整篇。挑選 2-4 張最適用的卡片即可，不用全部找出來。
...
八、投資智慧框架套用（股價生命週期判斷 + 適用的交易哲學 2-3 張卡片）
...
8. 投資智慧套用：在適用的段落引用 1 Cards/ 中的投資哲學卡片，用 [[wikilink]] 連結
...
YYYY-MM-DD-研究報告-[標的名稱].md

- 日期用今天的日期
- 個股範例：2026-04-22-研究報告-家登(3680).md
- 主題範例：2026-04-22-研究報告-先進封裝.md
...
用 mcp__obsidian-mcp-tools__create_vault_file 直接建立，不問使用者檔名或路徑。存完後進入 Step 5。
...
11. 禁止在未取得財務數據前就搜尋投資智慧卡片（必須等 Step 2 完成後才做 Step 2.5）
