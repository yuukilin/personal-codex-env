...
**Run this before starting.** Check which browser/performance tools are available in the current Codex session.

Preferred order in Codex:

1. Chrome DevTools MCP, if tools such as `navigate_page` or `performance_start_trace` are available.
2. Codex Browser or Chrome plugin tools, if the task is visual inspection, interaction, or authenticated browsing.
3. Playwright or Lighthouse from the local environment, if terminal-based measurement is the best available option.

If Chrome DevTools MCP is unavailable, do not stop immediately. Explain the limitation, use the best available browser tool, and clearly mark any metrics that could not be measured directly.
